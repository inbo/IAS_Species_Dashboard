...
The emergence score for the species will
be assessed for this period

```{r eval_years slider, echo=FALSE}
sliderInput(inputId = "evaluation_years", label = NULL, min = minjaar, max = maxjaar, value = c(maxjaar-2,maxjaar), step = 1, sep = "", dragRange = TRUE)
```

```{r em_status info}
renderText({
  em_status_text <- paste0(input$species, " was considered ","-!!placeholder em_status!!-", " between ",  min(input$evaluation_years), " & ", max(input$evaluation_years), ".") 
