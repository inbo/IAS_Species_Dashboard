...
test <- tryCatch(library(tidyverse), warning = function(w){"error"})
if(test == "error"){
  library(magrittr)
  library(tidyverse)
}
...
df_prot_areas <-
  ...
  mutate(spa = case_when(spa == FALSE ~ 0,
                         TRUE ~ 1),
         sac = case_when(sac == FALSE ~ 0,
                         TRUE ~ 1))
# Recent Species (>1950)
df_ts_sub <- df_ts %>% 
  ...
  summarise(
    ...
    sac_obs = sum(obs*sac, na.rm = TRUE),
    sac_cobs = sum(cobs*sac, na.rm = TRUE),
    spa_obs = sum(obs*spa, na.rm = TRUE),
    spa_cobs = sum(cobs*spa, na.rm = TRUE),
    ncells = sum(pa_obs, na.rm = TRUE),
    c_ncells = sum(pa_cobs, na.rm = TRUE),
    sac_ncells = sum(sac*pa_obs, na.rm = TRUE),
    spa_ncells = sum(spa*pa_obs, na.rm = TRUE),
    c_sac_ncells = sum(sac*pa_cobs, na.rm = TRUE),
    c_spa_ncells = sum(spa*pa_cobs, na.rm = TRUE)
  ) %>%
  ungroup()
...
table(kingdom_class$kingdomKey)
table(spec_names$kingdomKey)

spec_names <-
  spec_names %>%
  full_join(kingdom_class, by = c("kingdomKey", "classKey")) %>% 
  filter(!is.na(classKey))
...
names(df_ts_old)
summary(df_ts_old$cobs)
summary(df_ts_old$obs)
...
df_ts_sub_OeN <- df_ts_sub_OeN %>% 
  ...
  summarise(obs = sum(obs, na.rm = TRUE),
            cobs = sum(cobs, na.rm = TRUE),
            sac_obs = sum(sac_obs, na.rm = TRUE),
            sac_cobs = sum(sac_cobs, na.rm = TRUE),
            spa_obs = sum(spa_obs, na.rm = TRUE),
            spa_cobs = sum(spa_cobs, na.rm = TRUE),
            ncells = sum(ncells, na.rm = TRUE),
            c_ncells = sum(c_ncells, na.rm = TRUE),
            sac_ncells = sum(sac_ncells),
            spa_ncells = sum(spa_ncells),
            c_sac_ncells = sum(c_sac_ncells),
            c_spa_ncells = sum(c_spa_ncells),
...
stats_merged <- stats_merged %>%
  group_by(gbfp_SN, bioregion, tx) %>%
  summarise(utm1_n = sum(utm1, na.rm = TRUE),
            utm2 = sum(utm2, na.rm = TRUE),
            utm5 = sum(utm5, na.rm = TRUE),
            utm10 = sum(utm10, na.rm = TRUE),
            sac_perc = sum((sac_perc*utm1)/utm1_n, na.rm = TRUE)) %>% 
  rename(utm1 = utm1_n)
