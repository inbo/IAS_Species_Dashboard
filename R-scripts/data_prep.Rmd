...
df_ts_sub <- df_ts %>% 
  ...
  mutate(sac_obs = obs*sac,
         sac_cobs = cobs*sac,
         spa_obs = obs*spa,
         spa_cobs = cobs*spa,
         sac_ncells = sac*pa_obs,
         spa_ncells = spa*pa_obs,
         c_sac_ncells = sac*pa_cobs,
         c_spa_ncells = spa*pa_cobs) %>% 
  ...
  summarise(
    ...
    sac_ncells = sum(sac_ncells, na.rm = TRUE),
    spa_ncells = sum(spa_ncells, na.rm = TRUE),
    c_sac_ncells = sum(c_sac_ncells, na.rm = TRUE),
    c_spa_ncells = sum(c_spa_ncells, na.rm = TRUE)
  ) %>% 
  ungroup()
